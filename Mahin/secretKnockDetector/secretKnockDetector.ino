...
unsigned long storeMultipleKnock[1][4] = {{0,0,0,1}};
...
void loop() {
    ...
    //if switch is pressed new knock's time interval will be stored in the knockTimeStored array
    if(switchReading==0){
...
{   //after taking 4 knocks, storing time interval becomes terminated untill switch is pressed further
    if(count>=maxKnockInterval){  
